...
export default function Wizard({ onSuccess }) {
  ...
  return (
    <div className="bg-[#F7F6F2] text-on-surface">
      <header className="fixed top-0 w-full z-50 bg-[#2F2F2F] h-20 shadow-[0px_20px_40px_rgba(27,28,26,0.12)] flex items-center px-8">
        <div className="max-w-7xl mx-auto w-full flex justify-between items-center">
        <div className="flex items-center gap-4">
        <div className="w-10 h-10 bg-primary-container rounded-lg flex items-center justify-center">
        <span className="material-symbols-outlined text-[#2F2F2F]" style={{ fontVariationSettings: "'FILL' 1" }}>account_balance</span>
        </div>
        <h1 className="text-2xl font-extrabold tracking-tight text-white">Grant<span className="text-[#F4C84A]">IQ</span></h1>
        </div>
        <div className="hidden md:flex items-center gap-8">
        <div className="flex flex-col items-end">
        <span className="text-[10px] uppercase tracking-widest text-stone-400 font-bold">Profile Completion</span>
        <div className="flex items-center gap-3">
        <div className="w-48 h-1.5 bg-stone-700 rounded-full overflow-hidden">
        <div className="h-full bg-primary-container" style={{ width: `${progressPercent}%`, transition: 'width 0.5s ease' }}></div>
        </div>
        <span className="text-white font-bold text-sm">{progressPercent}%</span>
        </div>
        </div>
        <div className="h-10 w-10 rounded-full bg-stone-700 flex items-center justify-center text-white">
